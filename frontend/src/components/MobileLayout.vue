...
// Flatten nested sidebar links for mobile display
const flatSidebarLinks = computed(() => {
        let flat = []
        sidebarLinks.value.forEach((group) => {
                if (group.items && Array.isArray(group.items)) {
                        flat = flat.concat(group.items)
                } else if (group.icon) {
                        // If it's already a flat item (has icon directly)
                        flat.push(group)
                }
        })
        return flat
...
const addPrograms = async () => {
        ...
        // Find Learning group and add Programs to it
        const learningGroup = sidebarLinks.value.find(g => g.label === 'Learning')
        if (learningGroup && learningGroup.items) {
